...
from model.mask_refiner import MaskRefiner
from model.auto_arm_plus import AutoARMPlus
...
# maskrefiner_half = MaskRefiner(model_ckp_path=os.path.join(current_dir, "model/MaskRefiner/half_body"), device="cuda")
# maskrefiner_full = MaskRefiner(model_ckp_path=os.path.join(current_dir, "model/MaskRefiner/full_body"), device="cuda")
...
def predict(person_image, cloth_image, category, image_size=(1024, 512), device="cuda"):   
    ...
    if category != "overall":
        pred_mask_image = autoarm_plus_model.predict_from_original(
          person_image=person_image,
          clothing_image=cloth_image,
    ...
    with gr.Blocks(title="Auto ARM") as demo:
        with gr.Row():
            ...
            with gr.Column(scale=1, min_width=250):
                ...
                with gr.Row():
                    with gr.Column():
                        root_path = os.path.join(current_dir, "resource/demo/example")
