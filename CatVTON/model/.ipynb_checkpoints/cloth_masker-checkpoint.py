...
class AutoMasker:
    def __init__(
        ...
        # mask_refiner_ckpt='/home/jupyter/vinhdq_phucnph/CatVTON/model/MaskRefiner/epoch5',
        device='cuda'):
        np.random.seed(0)
        torch.manual_seed(0)
    ...
    @staticmethod
    def cloth_agnostic_mask_complement(
        densepose_mask: Image.Image,
        schp_lip_mask: Image.Image,
        schp_atr_mask: Image.Image,
        part: str='overall',
    ...
    def create_refined_mask(
        ...
        # complement_mask = self.cloth_agnostic_mask_complement(
        ...
        # complement_mask = transforms.Resize(image_size)(complement_mask.convert("L"))
        ...
        # complement_mask = np.array(complement_mask)
        predicted_mask = np.array(predicted_mask)
        
        # composed_mask_binary = (complement_mask == complement_mask.max()) | (predicted_mask == predicted_mask.max())
        ...
        cloth_protect_area = part_mask_of(PROTECT_CLOTH_PARTS_CUSTOM[part]['LIP'], schp_lip_mask, LIP_MAPPING) | \
            part_mask_of(PROTECT_CLOTH_PARTS_CUSTOM[part]['ATR'], schp_atr_mask, ATR_MAPPING)
        
        accessory_protect_area = part_mask_of((accessory_parts := ['Hat', 'Glove', 'Sunglasses', 'Bag', 'Left-shoe', 'Right-shoe', 'Scarf']), schp_lip_mask, LIP_MAPPING) | \
            part_mask_of(accessory_parts, schp_atr_mask, ATR_MAPPING) 
        
        # weak_protect_area = body_protect_area | cloth_protect_area | hair_protect_area | strong_protect_area | accessory_protect_area
#         
        weak_protect_area = body_protect_area | hair_protect_area | strong_protect_area | accessory_protect_area
        
        MASK_CLOTH_PARTS_CUSTOM = {
        ...
        # print("strong_mask_area")
        
        strong_mask_area = part_mask_of(MASK_CLOTH_PARTS_CUSTOM[part], schp_lip_mask, LIP_MAPPING) | \
            part_mask_of(MASK_CLOTH_PARTS_CUSTOM[part], schp_atr_mask, ATR_MAPPING)
        
        MASK_DENSE_PARTS_CUSTOM = {
            'upper': ["torso"],
            'lower': ['thighs', 'legs'],
            'overall': ['torso', 'thighs', 'legs', 'big arms', 'forearms'],
            'inner': ['torso'],
        ...
        mask_dense_area = part_mask_of(MASK_DENSE_PARTS_CUSTOM[part], densepose_mask, DENSE_INDEX_MAP)
        # print("composed_mask_binary")
        
        strong_mask_area = hull_mask(strong_mask_area * 255) // 255  # Convex Hull to expand the mask area
        strong_mask_area = cv2.GaussianBlur(strong_mask_area * 255, (kernal_size, kernal_size), 0)
        strong_mask_area[strong_mask_area < 25] = 0
        strong_mask_area[strong_mask_area >= 25] = 1
        
        strong_mask_area = cv2.dilate(strong_mask_area, dilate_kernel, iterations=1)
        ...
        return {
            ...
            "weak_protect_area": weak_protect_area,
            "body_protect_area": body_protect_area,
            "cloth_protect_area": cloth_protect_area,
