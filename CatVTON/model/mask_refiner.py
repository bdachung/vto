...
class MaskRefiner:
    def __init__(self, model_ckp_path="/home/jupyter/vinhdq_phucnph/CatVTON/model/MaskRefiner/epoch5", device="gpu"):
        ...
        unet.to(device)
        self.unet = unet
        self.target_cloth_masker = TargetClothMasker()
        self.skeleton_extractor = SkeletonExtractor()
        self.device = device
        
    def predict_from_original(self, 
                              person_image: Union[str, Image.Image], 
                              cloth_image: Union[str, Image.Image],
        ...
        if isinstance(agnostic_mask_image, str):
            agnostic_mask_image = agnostic_mask_image.open(agnostic_mask_image)
            
        clothing_mask_image = self.target_cloth_masker(cloth_image)
        skeleton_points, skeleton_image = self.skeleton_extractor(person_image)
        pose_image = Image.fromarray(np.uint8(skeleton_image)).convert("RGB")
        
        result = self(clothing_mask_image, agnostic_mask_image, pose_image, image_size, is_full)
        
        return {
            "cloth_mask": clothing_mask_image,
    ...
    def __call__(self, clothing_mask_image: Union[str, Image.Image], 
        ...
        transform_2 = transforms.Grayscale(num_output_channels=1)
    
        clothing_mask_image = transform_2(transform_1(clothing_mask_image))
        agnostic_mask_image = transform_2(transform_1(agnostic_mask_image))
        ...
        with torch.no_grad():
            input = torch.cat([clothing_mask_image, agnostic_mask_image, pose_image], dim=1).to(self.device)
