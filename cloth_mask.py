...
from openpose_integration import OpenPose  # Import the OpenPose class
...
def parse_args():
    ...
    parser.add_argument("--resize_width", type=int, default=38, help="Width to resize the image and mask.")
    parser.add_argument("--resize_height", type=int, default=, help="Height to resize the image and mask.")
...
def process_carvekit(cloth_path, interface, output_dir):
    ...
    # Save mask
    mask_output_path = os.path.join(output_dir, os.path.basename(cloth_path).replace('.jpg', '_carvekit_mask.png').replace('.jpeg', '_carvekit_mask.png'))
